...
public static class SeedData
{
    public static void Initialize(IServiceProvider serviceProvider)
    {
            ...
            serviceProvider.GetRequiredService<DbContextOptions<ApplicationDbContext>>()))
        {
            ...
            {
                context.UserAccounts.Add(new UserAccountData { Id = 1, FirstName = "seeduser", LastName = "seeduser", Email = "seed", Username="seeduser", Password = "seeduser" });
                context.SaveChanges();
            }
            if (!context.MealPlans.Any())
            {
                context.MealPlans.AddRange(
                    new MealPlanData { DateOfMeal = DateTime.Today, UserId = 1, Note = "Test meal plan" },
                    new MealPlanData { DateOfMeal = DateTime.Today, UserId = 1, Note = "Another test meal plan" }
                );
                context.SaveChanges();
            }
